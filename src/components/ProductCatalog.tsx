              ...
              <div className="flex items-center justify-between mb-10">
                ...
                {(selectedCategory !== "All" || selectedBrand !== "All") && (
                  <button 
                    ...
                    className="text-[9px] font-black text-ske-emerald uppercase border-b-2 border-ske-emerald pb-1"
                  >
                    Reset
                  </button>
                )}
          ...
          <main className="lg:col-span-9">
            ...
            {filteredProducts.length === 0 && (
              <div className="py-32 text-center border-2 border-dashed border-gray-100">
                ...
                <h3 className="text-xl font-black text-ske-dark uppercase mb-2">No Products Found</h3>
                <p className="text-sm text-ske-blue/40 font-medium">Try adjusting your filters or search query.</p>
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-3 mt-24 mb-12">
                <button 
                  ...
                  className="w-14 h-14 border-2 border-gray-100 flex items-center justify-center bg-white text-ske-blue hover:border-ske-emerald disabled:opacity-20 disabled:hover:border-gray-100 transition-all shadow-sm"
                >
                  <ChevronLeft size={24} />
                </button>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button 
                    ...
                    className={`w-14 h-14 text-xs font-black border-2 transition-all shadow-sm ${
                      currentPage === i + 1 
                        ? 'bg-ske-emerald border-ske-emerald text-white shadow-ske-emerald/20 shadow-lg' 
        ...
        {isDetailModalOpen && activeProduct && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-8">
            <motion.div 
              ...
              className="absolute inset-0 bg-ske-dark/90 backdrop-blur-xl" 
            />
            ...
            <motion.div 
              ...
              className="relative w-full max-w-[1200px] bg-white shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col md:flex-row overflow-hidden"
            >
              {/* Image Section */}
              <div className="w-full md:w-1/2 bg-[#F8F9FB] p-12 flex items-center justify-center relative overflow-hidden group">
                <div className="absolute top-10 left-10 text-[100px] font-black text-gray-100/50 leading-none select-none uppercase tracking-tighter">
                  {activeProduct.brand}
                </div>
                <img 
                  src={activeProduct.image} 
                  alt={activeProduct.name} 
                  className="relative z-10 w-full h-full object-contain mix-blend-multiply drop-shadow-[0_20px_50px_rgba(0,0,0,0.15)] group-hover:scale-105 transition-transform duration-700" 
                  referrerPolicy="no-referrer" 
                />
                <button onClick={() => setIsDetailModalOpen(false)} className="md:hidden absolute top-6 right-6 text-ske-dark p-2 border border-gray-200 bg-white">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col bg-white">
                <div className="hidden md:flex justify-end mb-8">
                  <button onClick={() => setIsDetailModalOpen(false)} className="group flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-ske-blue/40 hover:text-ske-emerald transition-colors">
                    Close Catalog <X size={20} />
                  </button>
                </div>

                <div className="flex-grow">
                  <div className="flex items-center gap-3 mb-6">
                    <span className="bg-ske-emerald text-white text-[9px] font-black uppercase tracking-widest px-3 py-1">{activeProduct.brand}</span>
                    <span className="text-[9px] font-black text-ske-blue/30 uppercase tracking-widest">{activeProduct.category}</span>
                  </div>
                  
                  <h2 className="text-3xl md:text-5xl font-black text-ske-dark uppercase leading-[0.9] tracking-tighter mb-8 italic">
                    {activeProduct.name}
                  </h2>
                  
                  <div className="flex items-baseline gap-4 mb-12">
                    <span className="text-4xl font-black text-ske-dark">{formatPrice(activeProduct.price)}</span>
                    <span className="text-[10px] font-bold text-ske-emerald uppercase tracking-widest underline underline-offset-4 decoration-2">Inc. PPN 11%</span>
                  </div>

                  <div className="grid grid-cols-2 gap-10 mb-12 border-y border-gray-100 py-10">
                    <div>
                      <h4 className="text-[10px] font-black text-ske-blue uppercase tracking-widest mb-4 flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-ske-emerald" /> Specifications
                      </h4>
                      <div className="space-y-3">
                         <div className="flex flex-col">
                           <span className="text-[9px] font-bold text-ske-blue/30 uppercase mb-1">Dimensions (LxWxH)</span>
                           <span className="text-xs font-black text-ske-dark">{activeProduct.specs.dimension}</span>
                         </div>
                         <div className="flex flex-col">
                           <span className="text-[9px] font-bold text-ske-blue/30 uppercase mb-1">Capacity / Weight</span>
                           <span className="text-xs font-black text-ske-dark">{activeProduct.specs.capacity}</span>
                         </div>
                      </div>
                    </div>
                    <div>
                      <h4 className="text-[10px] font-black text-ske-blue uppercase tracking-widest mb-4 flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-ske-emerald" /> Service Info
                      </h4>
                      <ul className="space-y-2">
                        <li className="flex items-center gap-2 text-[9px] font-bold text-ske-blue uppercase tracking-tighter">
                          <CheckCircle size={12} className="text-ske-emerald" /> Garansi 1 Tahun
                        </li>
                        <li className="flex items-center gap-2 text-[9px] font-bold text-ske-blue uppercase tracking-tighter">
                          <CheckCircle size={12} className="text-ske-emerald" /> Ready Stock Jabodetabek
                        </li>
                        <li className="flex items-center gap-2 text-[9px] font-bold text-ske-blue uppercase tracking-tighter">
                          <CheckCircle size={12} className="text-ske-emerald" /> Instalasi & Training
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 bg-ske-emerald text-white py-6 text-xs font-black uppercase tracking-[0.3em] transition-all hover:bg-ske-dark shadow-xl shadow-ske-emerald/20 flex items-center justify-center gap-4 group"
                  >
                    Request Quatation <MessageCircle size={18} className="group-hover:rotate-12 transition-transform" />
                  </button>
                </div>
              </div>
            </motion.div>
        ...
        {isInquiryModalOpen && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            <motion.div 
              ...
              className="absolute inset-0 bg-ske-dark/95 backdrop-blur-md" 
            />
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }} 
              exit={{ opacity: 0, scale: 0.9 }} 
              className="relative w-full max-w-[550px] bg-white p-10 md:p-14 border-t-8 border-ske-emerald"
            >
              <div className="flex justify-between items-center mb-10">
                <div>
                  <h2 className="text-2xl font-black text-ske-dark uppercase tracking-tight italic leading-none">Inquiry Form</h2>
                  <p className="text-[10px] font-bold text-ske-blue/40 uppercase tracking-widest mt-2">Personalize your industrial equipment inquiry</p>
                </div>
              ...
              {!showSuccess ? (
                <div className="space-y-8">
                  ...
                  <div className="flex items-center gap-6 p-4 bg-gray-50 border border-gray-100">
                    <div className="w-16 h-16 bg-white p-2 border border-gray-200">
                      <img src={activeProduct?.image} alt="" className="w-full h-full object-contain mix-blend-multiply" />
                    </div>
                    <div>
                      <div className="text-[8px] font-black text-ske-emerald uppercase tracking-[0.2em] mb-1">{activeProduct?.brand}</div>
                      <div className="text-[11px] font-black text-ske-dark uppercase">{activeProduct?.name}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 gap-4">
                      <div className="relative">
                        <label className="text-[9px] font-black text-ske-blue uppercase tracking-widest absolute -top-2 left-4 bg-white px-2">Nama Lengkap</label>
                        <input 
                          type="text" 
                          value={formData.name}
                          onChange={(e) => setFormData({...formData, name: e.target.value})}
                          className="w-full border-2 border-gray-100 p-4 text-xs font-bold outline-none focus:border-ske-emerald transition-all" 
                          placeholder="Contoh: Bpk. Kurniawan"
                        />
                      </div>
                      <div className="relative">
                        <label className="text-[9px] font-black text-ske-blue uppercase tracking-widest absolute -top-2 left-4 bg-white px-2">Lokasi / Alamat Resto</label>
                        <input 
                          type="text" 
                    ...
                    <div className="relative">
                      <label className="text-[9px] font-black text-ske-blue uppercase tracking-widest absolute -top-2 left-4 bg-white px-2">Pesan Tambahan (Opsional)</label>
                      <textarea 
                        rows={3}
                        value={formData.inquiry}
                        onChange={(e) => setFormData({...formData, inquiry: e.target.value})}
                        className="w-full border-2 border-gray-100 p-4 text-xs font-bold outline-none focus:border-ske-emerald transition-all resize-none" 
                        placeholder="Tanyakan spesifikasi, stok, atau ongkir..."
                      />
                    </div>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-ske-emerald text-white py-5 text-xs font-black uppercase tracking-[0.3em] shadow-xl shadow-ske-emerald/20 hover:bg-ske-dark transition-all disabled:opacity-30 disabled:grayscale flex items-center justify-center gap-4"
                  >
                    Kirim via WhatsApp <ExternalLink size={16} />
                  </button>
                  
                  <p className="text-center text-[8px] font-bold text-ske-blue/30 uppercase tracking-[0.2em]">Our experts will respond within 15 minutes</p>
                </div>
              ) : (
                <motion.div 
                ...
                >
                  <div className="w-20 h-20 bg-ske-emerald/10 rounded-full flex items-center justify-center mx-auto mb-6">
                    <CheckCircle size={40} className="text-ske-emerald" />
                  </div>
                  <h3 className="text-2xl font-black text-ske-dark uppercase mb-2">Redirecting...</h3>
                  <p className="text-xs text-ske-blue/40 font-medium max-w-[280px] mx-auto leading-relaxed">
                    Sistem sedang membuka WhatsApp. Mohon selesaikan pengiriman pesan di aplikasi WhatsApp Anda.
                  </p>
                  <button onClick={() => setShowSuccess(false)} className="mt-10 text-[10px] font-black text-ske-emerald uppercase border-b-2 border-ske-emerald pb-1">Kirim Pesan Lagi</button>
                </motion.div>
              )}
            </motion.div>
          </div>
